...
export const DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE = [
  ...
  "- Final disposition checklist: mark `done` when complete; use `in_review` only with a real reviewer, approval, interaction, or monitor path; use `blocked` only with first-class blockers or a named unblock owner/action (if the operator must answer or decide something, file a question card (ask_user_questions or request_confirmation interaction) first, because an agent's `blocked` with no linked blocking task, pending question card or linked approval is refused); create delegated follow-up issues with blockers when another agent owns the next step; keep `in_progress` only when a live continuation path exists.",
  ...
  "- If blocked, mark the issue blocked and name the unblock owner and action. If another task must finish first, link it with blockedByIssueIds. If the operator must answer or decide something, first create an ask_user_questions or request_confirmation interaction; questions written only in a comment give the operator no way to answer, and the server refuses `blocked` without a linked blocking task, pending question card or linked approval.",
...
/**
 * One-paragraph restatement of the heartbeat execution contract. The default
 * heartbeat prompt template (DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE) spells
 * the same contract out in full; adapters that send both in one prompt can
 * pass `omitExecutionContract` to avoid carrying it twice on every turn
 * (DUR-3943).
 */
export const PAPERCLIP_WAKE_EXECUTION_CONTRACT_LINE =
  "Execution contract: take concrete action in this heartbeat when the issue is actionable; do not stop at a plan unless planning was requested. Leave durable progress and then give the issue a clear final disposition before ending the heartbeat: `done`, `in_review` with a real reviewer/approval/interaction path, `blocked` with first-class blockers or a named unblock owner/action (if the operator must answer or decide something, file a question card (ask_user_questions or request_confirmation interaction) first, because an agent's `blocked` with no linked blocking task, pending question card or linked approval is refused), delegated follow-up issues with blockers, or `in_progress` only when a live continuation path exists. Use child issues for long or parallel delegated work instead of polling. Comments, documents, screenshots, work products, and `Remaining` bullets are evidence, not valid liveness paths by themselves.";

export function renderPaperclipWakePrompt(
  value: unknown,
  options: { resumedSession?: boolean; omitExecutionContract?: boolean } = {},
): string {
  const normalized = normalizePaperclipWakePayload(value);
  if (!normalized) return "";
  const resumedSession = options.resumedSession === true;
  const executionContractLines = options.omitExecutionContract === true
    ? []
    : [PAPERCLIP_WAKE_EXECUTION_CONTRACT_LINE, ""];
