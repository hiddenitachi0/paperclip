...
describe("createDataConnectionSchema", () => {
  ...
  it("accepts SFTP with a password or a private key, port 22 by default, and a full read path", () => {
    ...
    const withKey = createDataConnectionSchema.parse({
      ...
      port: 2222,
      username: "paperclip",
      remotePath: "/rapporter/2026",
      credential: { kind: "private_key", privateKey: PEM, passphrase: "pp" },
    });
    expect(withKey).toMatchObject({ port: 2222, credential: { kind: "private_key", privateKey: PEM, passphrase: "pp" } });
    for (const bad of [
      { host: "10.0.0.5" },
      { host: "filserver" },
      { remotePath: "rapporter" },
      { remotePath: "/rapporter/../etc" },
...
describe("credential kinds", () => {
  it("every credential kind belongs to exactly one source kind, and the union covers them all", () => {
    const seen = new Map<string, string>();
    for (const kind of DATA_CONNECTION_KINDS) {
      for (const credentialKind of DATA_CONNECTION_CREDENTIAL_KINDS_BY_KIND[kind]) {
        expect(seen.has(credentialKind), `${credentialKind} listed for ${seen.get(credentialKind)} and ${kind}`).toBe(false);
        seen.set(credentialKind, kind);
      }
    }
    expect([...seen.keys()].sort()).toEqual([...DATA_CONNECTION_CREDENTIAL_KINDS].sort());
  ...
  it("only Shopify is readable today", () => {
    expect(SUPPORTED_DATA_CONNECTION_KINDS).toEqual(["shopify"]);
  });
});
