...
// DUR-3945 (DUR-244 item 3): proves migration 0160's two LOGIN roles behave
...
describeEmbeddedPostgres("DUR-3945: RLS login roles (migration 0160)", () => {
  let db!: Db;
  ...
  it("the scoped login can reach the instance-wide tables 0149 never granted (auth users, cross_company_access_log)", async () => {
    await withPool(scopedUrl, async (scopedPool) => {
      // Would be "permission denied for table user" before 0160.
  ...
  it("every public table with a company_id column carries the paperclip_company_scope policy (drift guard for future migrations)", async () => {
    ...
    expect(
      ...
      "a migration added a tenant table without an RLS policy -- add a paperclip_company_scope policy for it in that migration (see 0149/0160)",
    ).toEqual([]);
  });
