...
#   1. server-secrets-handoff.sh works out the server's keys (the name list
#      written at image build time from @paperclipai/adapter-utils/
#      server-env-secrets), taking each value from the root-only secrets file
#      /run/secrets/paperclip_server when that has one, otherwise from the
#      environment;
...
#      started with.
# Everything this runs as root is root-owned and outside /app (which the
# `node` user -- i.e. every agent -- can write): the helper, the name list
# and the system tools. It never runs Node as root.
...
# exactly as before.
SECRETS_HANDOFF_SCRIPT=/usr/local/lib/paperclip/server-secrets-handoff.sh
SECRETS_NAMES_FILE=/usr/local/share/paperclip/server-secret-names
SECRETS_FILE=/run/secrets/paperclip_server

handoff=""
case " $* " in
    *" server/dist/index.js "*|*" /app/server/dist/index.js "*)
        if ! handoff="$(/bin/sh "$SECRETS_HANDOFF_SCRIPT" "$SECRETS_FILE" "$SECRETS_NAMES_FILE")"; then
