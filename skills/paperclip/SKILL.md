...
**Personality never applies to what you send the operator (DUR-61).** If your agent has a saved "how this agent talks" voice, it only changes the wording of what you write in task comments, drafts, and status updates. It never applies to `request_board_approval` payloads, `request_confirmation`/`ask_user_questions`/`suggest_tasks` prompts, or any `plainSummary` field — write those exactly as this skill specifies, in plain, literal words, with no personality flourish, regardless of what your voice box says. There is no server-side check that rewrites your own summary text for these calls, so this boundary is on you: hold the line yourself.
